...
Taken from CrossVit : https://github.com/IBM/CrossViT/blob/main/models/crossvit.py
...
class PatchEmbed(nn.Module):
    ...
    ### To Be changed to get two categories of patches , large and small for global and local ###
        """
        img_size : size of the input image (TO BE CHECKED)
        patch_size : Size of each patch (We should initalize two ,  one for large and one for small)
        in_chans : RGB --> 3
    ...
    def forward(self, x):
        ...
        x = self.proj(x).flatten(2).transpose(1, 2) # (B ,  C , H , W ) --> (B , H*W , C)
        return x
...
class CrossAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # learnable key , query, and value weights
    ...
    def forward(self, x):
        ...
        B, N, C = x.shape # shape of patched flattend input
...
class CrossAttentionBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        if has_mlp:  # I think this is for prediction/classification , so in our case shoulde be FALSE
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=(224, 224), patch_size=(8, 16), in_chans=3, num_classes=1000, embed_dim=(192, 384), depth=([1, 3, 1], [1, 3, 1], [1, 3, 1]),
        ...
        self.num_branches = len(patch_size) # how many patch categories (short and large patches)

        self.patch_embed = nn.ModuleList()
        if hybrid_backbone is None:
            self.pos_embed = nn.ParameterList([nn.Parameter(torch.zeros(1, 1 + num_patches[i], embed_dim[i])) for i in range(self.num_branches)]) # initialize pos embed with zeroes
            for im_s, p, d in zip(img_size, patch_size, embed_dim):
                self.patch_embed.append(PatchEmbed(img_size=im_s, patch_size=p, in_chans=in_chans, embed_dim=d, multi_conv=multi_conv)) # patch embedd
        else:
            ...
            for idx, (im_s, p, d) in enumerate(zip(img_size, patch_size, embed_dim)):
                ...
                self.pos_embed.append(nn.Parameter(data=get_sinusoid_encoding(n_position=1 + num_patches[idx], d_hid=embed_dim[idx]), requires_grad=False)) # position embed initalize by sine function

            del self.pos_embed
            self.pos_embed = nn.ParameterList([nn.Parameter(torch.zeros(1, 1 + num_patches[i], embed_dim[i])) for i in range(self.num_branches)])

        self.cls_token = nn.ParameterList([nn.Parameter(torch.zeros(1, 1, embed_dim[i])) for i in range(self.num_branches)]) # initialize class token with 1
